	...
	return (
		<div className="flex flex-col items-center min-w-[550px] h-full bg-cover shadow-2xl bg-login-bg bg-center max-h-full overflow-auto pb-10">
			<div className="my-14 flex flex-row w-[800px] justify-center items-center relative">
				<button className="p-3 hover:bg-levelHard bg-primary rounded-md absolute left-0" onClick={() => navigate("/subjects")}>
				...
				{({ errors, values, setFieldValue }) => (
					<>
						<div className="grid grid-cols-2 gap-8 gap-y-3 w-[800px]">
							<TextFieldCustom label="Mã môn học*" text={values.subjectId} onChange={text => setFieldValue("subjectId", text)} />
							<TextFieldCustom label="Tên môn học*" text={values.subjectId} onChange={text => setFieldValue("subjectId", text)} />
							<TextFieldCustom label="Số tín chỉ*" text={values.subjectId} onChange={text => setFieldValue("subjectId", text)} />
						</div>
